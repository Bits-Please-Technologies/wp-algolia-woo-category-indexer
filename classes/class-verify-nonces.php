...
if ( ! class_exists( 'Algolia_Verify_Nonces' ) ) {
	...
	class Algolia_Verify_Nonces {
		...
		public static function verify_settings_nonce() {
			 ...
			 */
			$settings_nonce = filter_input( INPUT_POST, 'algolia_woo_category_indexer_admin_api_nonce_name', FILTER_DEFAULT );
			 ...
			 * @return bool
			 */
		public static function verify_send_categories_nonce() {
			/**
			 * Filter incoming nonces and values
			 */
			$send_categories_nonce      = filter_input( INPUT_POST, 'send_categories_to_algolia_nonce_name', FILTER_DEFAULT );
			$send_categories_to_algolia = filter_input( INPUT_POST, 'send_categories_to_algolia', FILTER_DEFAULT );
			...
			if ( ! wp_verify_nonce( $send_categories_nonce, 'send_categories_to_algolia_nonce_action' ) && isset( $send_categories_to_algolia ) ) {
				wp_die( esc_html__( 'Action is not allowed.', 'algolia-woo-category-indexer' ), esc_html__( 'Error!', 'algolia-woo-category-indexer' ) );
			}
